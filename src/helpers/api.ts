...
export const handleCancellation = async (id:number):Promise<IResponse> =>{
...
export const handleRequestAccept = async (id:number):Promise<IResponse>=>{
  const response = await Axios.patch('/requests/accept/'+id)
  return response.data
}
export const handleRequestDecline = async (id:number):Promise<IResponse>=>{
